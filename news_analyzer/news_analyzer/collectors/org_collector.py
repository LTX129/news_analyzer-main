...
class OrgCollector:
    ...
    def _fetch_world_bank(self, source: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if isinstance(articles, dict):
            iterable = articles.values()
        else:
            iterable = articles or []
        ...
        for entry in iterable:
            ...
            if not title or not link:
                continue
            description = entry.get("abstract") or entry.get("content") or entry.get("body") or ""
            pub_date = entry.get("date") or entry.get("publishedDate") or ""
    ...
    def _fetch_unctad(self, source: Dict[str, Any]) -> List[Dict[str, Any]]:
        return self._fetch_html_links(
            source,
            container_selector=".views-row, article, div.node",
            link_filter="/news/"
        )
    ...
    def _fetch_html_links(
        ...
        for a in links:
            ...
            description = ""
            pub_date = ""
            items.append(self._build_item(title, full_link, description, pub_date, source))
        return items
