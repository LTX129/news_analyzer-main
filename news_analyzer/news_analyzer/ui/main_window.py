...
class MainWindow(QMainWindow):
    ...
    def __init__(self, storage, rss_collector=None, org_collector=None):
        ...
        # 启动时加载数据库中已有的新闻
        self._load_cached_news_on_start()
    ...
    def _load_cached_news_on_start(self):
        """启动时加载数据库中的最新新闻"""
        try:
            news_items = self.storage.load_news()
            if not news_items:
    ...
    def _merge_news(self, *news_lists):
        """合并多源新闻并按链接去重"""
        unique = {}
    ...
    def refresh_news(self, source_url=None):
        ...
        try:
            ...
            # 从数据库按批次读取，确保与存储一致且无重复
            if batch_key:
                news_items = self.storage.load_news(batch_key)
            else:
                news_items = []
